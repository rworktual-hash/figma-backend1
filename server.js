...
// Helper to detect project type from a prompt string (for legacy endpoint)
function detectProjectTypeFromPrompt(prompt) {
    ...
    if (text.includes('finance') || text.includes('bank') || text.includes('money')) return 'finance';
    if (text.includes('health') || text.includes('medical') || text.includes('clinic')) return 'healthcare';
    if (text.includes('education') || text.includes('school') || text.includes('learn')) return 'education';
    if (text.includes('shop') || text.includes('store') || text.includes('ecommerce')) return 'ecommerce';
    if (text.includes('nature') || text.includes('eco') || text.includes('green')) return 'nature';
    if (text.includes('luxury') || text.includes('premium')) return 'luxury';
    if (text.includes('creative') || text.includes('design') || text.includes('art')) return 'creative';
...
function createFallbackDesign(prompt) {
    ...
    let projectType = detectProjectTypeFromPrompt(prompt);
            ...
            children: [
                ...
                },
                {
                    ...
                    children: [
                        ...
                        { type: "image", name: "Hero Image", width: 600, height: 300, x: 700, y: 100, borderRadius: 12 }
                    ]
                },
                {
                    type: "frame",
        ...
        design = {
            frames: [{
                ...
                children: [
                    ...
                    // Add hero image on left
                    { type: "image", name: "Hero Image", width: 600, height: 500, x: 100, y: 200, borderRadius: 12 }
                ]
            }]
        };
    } else {
